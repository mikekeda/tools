...
def tool(request, slug, username=None):
    ...
    try:
        ...
        return render(
            request, slug + ".html", dict(active_page=slug, profile_user=user)
        )
...
class FlashcardsView(LoginRequiredMixin, View, GetUserMixin):
    ...
    def delete(self, request, pk, username=None):
        ...
        return JsonResponse({"redirect": reverse("flashcards"), "success": True})
...
class TasksView(LoginRequiredMixin, View, GetUserMixin):
    ...
    def post(self, request, username=None):
        ...
        if form.is_valid():
            first_task = (
                Task.objects.filter(user=user, status="todo").order_by("weight").first()
            )
...
class CalendarView(LoginRequiredMixin, View, GetUserMixin):
    def get(self, request, username=None):
        ...
        return render(
            request, "calendar.html", dict(profile_user=user, form=EventForm())
        )
...
class ProfileView(LoginRequiredMixin, View, GetUserMixin):
    ...
    def post(self, request, username=None):
        ...
        if any(
            ...
            if field == "phone_number" or field.startswith("palette_color_"):
                current_obj = profile
            else:
                current_obj = user
...
def user_events(request):
    ...
    if request.is_ajax():
        ...
        for event in events:
            ...
            local_time = timezone.localtime(event.start, user_timezone).strftime(
                "%H:%M"
            )
    ...
    def post(self, request, slug=None, username=None):
        ...
        if form.is_valid():
            ...
            try:
                ...
                langs = set(
                    re.findall('<code class="language-(.*?)">', code.text, re.DOTALL)
                )
...
class LinkView(View, GetUserMixin):
    def get(self, request, username=None):
        ...
        links = (
            Link.objects.select_related("category")
            .filter(user=user)
            .order_by("category__title", "weight")
        )
    ...
    def post(self, request, username=None):
        ...
        if link_id:
            ...
            link = Link(user=user)
            first_link = Link.objects.filter(user=user).order_by("weight").first()
